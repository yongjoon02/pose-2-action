...
def main():
    ...
    # 벡터화된 삼각측량 및 reprojection error 기반 필터링 함수
    ...
    # Intrinsic 및 Extrinsic 캘리브레이션 함수 (기존과 동일)
    ...
    # 2D Pose JSON 파일에서 좌표 불러오기 및 삼각측량 + 3D 시각화
    ...
    for frame_idx in range(num_frames):
        ...
        # inlier된 점들을 dict로 관리 (키: 2D 조인트 인덱스, 값: 3D 좌표)
        inlier_points = {
        ...
        for j_idx, coord_3d in inlier_points.items():
            joints_3d[j_idx] = coord_3d.tolist()
    ...
    # 최종적으로 3D 관절 좌표를 JSON에 저장
    output_3d_json_path = triang_config["output_3d_json"]
